...
| `prefetch_prime = on` | `UCACHE_PREFETCH_PRIME` | read a file's basket map as soon as it is opened, rather than waiting for your code to fetch it. That serves the metadata reads at the start of every file out of RAM, and the map is then ready for the file's first batch instead of its third. Does nothing until read-ahead has engaged on an earlier file |
| `prefetch_prime_first_fill = on` | `UCACHE_PREFETCH_PRIME_FIRST_FILL` | with the map in hand, also predict the file's FIRST batch. Right when your code reads a file from the beginning; a reader that gives each file handle its own range of entries starts somewhere else, and then the guess is fetched for nothing. Needs `prefetch_prime` |
