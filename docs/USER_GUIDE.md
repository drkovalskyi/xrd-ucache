...
| `prefetch_prime = off` | `UCACHE_PREFETCH_PRIME` | read a file's basket map as soon as it is opened, rather than at the first batch your code asks for. Off, and measured to be worth nothing on either a many-threaded or a single-threaded reader: the map was not being read late, so there is nothing to bring forward. Does nothing until read-ahead has engaged on an earlier file |
| `prefetch_prime_first_fill = on` | `UCACHE_PREFETCH_PRIME_FIRST_FILL` | with the map in hand, also predict the file's FIRST batch. Right when your code reads each file from the beginning; a reader that gives each file handle its own range of entries starts somewhere else, and then the guess is fetched for nothing. Needs `prefetch_prime` |
