...
| `prefetch_depth = 2` | `UCACHE_PREFETCH_DEPTH` | how many batches ahead to keep fetching. One is not enough on a busy origin: a batch takes about as long to fetch as it takes your code to process the one before it, so at depth 1 any slow read stalls the reader. `1` restores the older behaviour |
| `prefetch_prime = on` | `UCACHE_PREFETCH_PRIME` | read a file's basket map as soon as it is opened, rather than waiting for your code to fetch it. That serves the metadata reads at the start of every file out of RAM and lets the FIRST batch be predicted too — the one batch per file nothing else can cover. Does nothing until read-ahead has engaged on an earlier file |
| `prefetch_bridge_kb = 0` | `UCACHE_PREFETCH_BRIDGE_KB` | join predicted ranges separated by less than this into one request element. An origin charges far more for the number of pieces in a request than for their size, so reading across a small gap can be quicker than skipping it. The extra bytes are thrown away on arrival and never enter the cache; they cost bandwidth. Off by default |
