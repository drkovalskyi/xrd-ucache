...
| `ucache bench [PATH …] [--size SZ] [--phase-seconds S] [--streams 1,16,64] [--block KB] [--fill writers=N,block=SZ,file=SZ] [--log FILE\|--no-log]` | raw storage self-test of the cache dir (or of candidate dirs, to pick one): sequential and large-block read/write at each stream count, burst vs sustained write, scattered-write IOPS, random reads with latency percentiles, fsync/create/unlink. `-S` is the window of ONE stage (build gets 3×; the plan and estimated total print up front). Appends every run, with the machine, load and block device behind the path, to `./ucache-bench.txt` — see `docs/CACHE_MANAGEMENT.md` §8 |
